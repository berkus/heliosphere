...
func events(w http.ResponseWriter, r *http.Request) {
	...
	grouping := make(map[int64]*EventsProto)
	for _, event := range events {
		add(&event, grouping, player)
	}
...
func add(event *Event, grouping map[int64]*EventsProto, player *Player) {
	proto := proto(event, player)
	rounded_time := event.Date.Truncate(24 * time.Hour)
	rounded_msec := rounded_time.Unix()
	_, in := grouping[rounded_msec]
	if !in {
		grouping[rounded_msec] = &EventsProto{Date: stringify(rounded_time)}
	}
	grouping[rounded_msec].append(proto)
}
